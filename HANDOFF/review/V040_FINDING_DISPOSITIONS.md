...
| F10 unbounded ledger growth / disk DoS | PARTIAL | FIX IN PROGRESS | load cap LANDED (v2a-1); byte bounds LANDED (v2a-2); GPT V2A_PRECHECK: load-time sanitization + durable shrink-persist folded into v2c-2; save serialization + atomic writes (v2c-1 in flight); batch caller (1c in flight); residual: bounded-memory streaming parse of oversized legacy files = post-alpha (write-side caps + shrink-persist close growth) |
...
| NEW (1b review) lost-update race + corruption | save serialization LANDED (21095127); batch callers LANDED (d2497460) | FIXED pending v2c-2 tests + terminal verdict | save_lock-before-snapshot + unique tmp + sync_all + rename landed; load sanitization + durable shrink + concurrency tests = v2c-2 in flight |

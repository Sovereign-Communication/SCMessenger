...
| F10 unbounded ledger growth / disk DoS | NO-SHIP (GPT precheck 2026-07-29) | FIX IN PROGRESS (W1) | insert/load caps + bounds + atomic writes + shrink + recovery landed (v2a-1..v2c-3), BUT the 16MB guard is WARN-ONLY: load() still reads/parses the full oversized file before capping -- the enforcement-before-parse gap is W1. Also load() state replacement unsynchronized with saves (W1). Earlier CLOSED marking reverted |
...
| NEW (1b review) lost-update race + corruption | NO-SHIP (GPT precheck 2026-07-29) | FIX IN PROGRESS (W1) | save_lock + atomic writes + sanitization + shrink + recovery LANDED, BUT GPT correctly flagged two residual gaps: load() warns on oversized file then still reads/parses it (no hard limit before parse), and load() replaces entries WITHOUT holding save_lock (stale load can overwrite newer save). W1 agy packet remediates both + multi-manager contract + tests. Disposition prematurely marked CLOSED earlier -- reverted per GPT finding |

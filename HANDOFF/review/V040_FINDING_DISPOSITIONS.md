...
| F10 unbounded ledger growth / disk DoS | CLOSED | FIXED | load cap (v2a-1); byte bounds at ingest AND load (v2a-2 + v2c-2); save serialization + atomic durable writes (v2c-1); batch caller (1c); durable shrink-persist at load (v2c-2); corrupt-file recovery (v2c-3); W1: hard MAX_LEDGER_FILE_BYTES limit BEFORE parse -- oversized files quarantined + empty recovery (no data enters memory) |
...
| NEW (1b review) lost-update race + corruption | CLOSED | FIXED | save_lock + atomic durable writes (21095127); batch callers (d2497460); load sanitization + durable shrink + 5 persistence/concurrency tests (02efea70, Fusion: lock ordering/tmp race/shrink all SOUND); corrupt-JSON recovery + peer_id parity (v2c-3); W1: save_lock acquired in load() BEFORE entries replacement -- concurrent saves cannot snapshot stale pre-load data |

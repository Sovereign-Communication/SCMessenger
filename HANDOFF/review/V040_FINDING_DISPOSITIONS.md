...
| F12 ranking poison / floor placement | OPEN -- bounded magnitude but rank-1 still steerable | BOUNDED-NOT-FIXED: the recency clamp at mesh_routing.rs:516-528 caps the magnitude of an injected `seen_at` to now+300s, but `recipient_recency` is the primary descending sort key, so an attacker relay that re-asserts every <300s with `seen_at=now+300` still ranks FIRST among relays for any target it names, indefinitely -- honest relays report `seen_at<=now` and can never win. Residual risk: persistent priority hijack of relay ranking for attacker-named targets. Candidate remediations (pick one): (a) clamp to `min(seen_at, now)` so future timestamps cannot outrank honest relays; (b) demote `recipient_recency` below `relay_success_score` in the sort so proven success beats recency; (c) bucket `recipient_recency` into coarse windows (e.g. 5-min buckets) so a 300s lead cannot create strict ordering. | record_recipient_seen_via_relay_from_wire |

...
The function 'try_enable_bluetooth' is defined in 'cli\src\ble_daemon.rs' but is NEVER called anywhere in the repository. Your job is to fully integrate it into the overarching application loop. It may need to be wired into 1, 2, or 3+ different locations.

You MUST follow this strict analytical loop. Do not skip phases.

PHASE 1: CONTEXT GATHERING (Search & Ponder)
1. Open 'cli\src\ble_daemon.rs' and read the implementation of 'try_enable_bluetooth'. Understand its parameters, return type, and exact purpose.
...
3. Identify EVERY valid location in the codebase where 'try_enable_bluetooth' MUST be invoked to work fully. 

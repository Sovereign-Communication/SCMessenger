...
The function 'proptest_ratchet_forward_secrecy' is defined in 'core\src\crypto\proptest_harness.rs' but is NEVER called anywhere in the repository. Your job is to fully integrate it into the overarching application loop. It may need to be wired into 1, 2, or 3+ different locations.

You MUST follow this strict analytical loop. Do not skip phases.

PHASE 1: CONTEXT GATHERING (Search & Ponder)
1. Open 'core\src\crypto\proptest_harness.rs' and read the implementation of 'proptest_ratchet_forward_secrecy'. Understand its parameters, return type, and exact purpose.
...
3. Identify EVERY valid location in the codebase where 'proptest_ratchet_forward_secrecy' MUST be invoked to work fully. 

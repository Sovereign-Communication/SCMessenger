...
The function 'get_history_via_api' is defined in 'cli/src/api.rs' but is NEVER called anywhere in the repository. Your job is to fully integrate it into the overarching application loop. It may need to be wired into 1, 2, or 3+ different locations.

PHASE 1: CONTEXT GATHERING
1. Open 'cli/src/api.rs' and read the implementation of 'get_history_via_api'. Understand its parameters, return type, and exact purpose.
2. Use search tools to hunt for related concepts, APIs, UI buttons, or parent managers where similar functions are called.
3. Identify EVERY valid location in the codebase where 'get_history_via_api' MUST be invoked to work fully.

...
TARGET: core\src\drift\relay.rs

SYSTEM DIRECTIVE: COMPREHENSIVE DEAD-END RESOLUTION
The function 'generate_cover_traffic_if_due' is defined in 'core\src\drift\relay.rs' but is NEVER called anywhere in the repository. Your job is to fully integrate it into the overarching application loop. It may need to be wired into 1, 2, or 3+ different locations.

You MUST follow this strict analytical loop. Do not skip phases.

PHASE 1: CONTEXT GATHERING (Search & Ponder)
1. Open 'core\src\drift\relay.rs' and read the implementation of 'generate_cover_traffic_if_due'. Understand its parameters, return type, and exact purpose.
...
3. Identify EVERY valid location in the codebase where 'generate_cover_traffic_if_due' MUST be invoked to work fully. 

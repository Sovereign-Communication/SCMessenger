...
The function 'signed_registration_request_rejects_tampered_payload' is defined in 'core\src\transport\behaviour.rs' but is NEVER called anywhere in the repository. Your job is to fully integrate it into the overarching application loop. It may need to be wired into 1, 2, or 3+ different locations.

You MUST follow this strict analytical loop. Do not skip phases.

PHASE 1: CONTEXT GATHERING (Search & Ponder)
1. Open 'core\src\transport\behaviour.rs' and read the implementation of 'signed_registration_request_rejects_tampered_payload'. Understand its parameters, return type, and exact purpose.
...
3. Identify EVERY valid location in the codebase where 'signed_registration_request_rejects_tampered_payload' MUST be invoked to work fully. 

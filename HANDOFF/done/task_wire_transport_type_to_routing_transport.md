...
The function 'transport_type_to_routing_transport' is defined in 'core\src\transport\swarm.rs' but is NEVER called anywhere in the repository. Your job is to fully integrate it into the overarching application loop. It may need to be wired into 1, 2, or 3+ different locations.

You MUST follow this strict analytical loop. Do not skip phases.

PHASE 1: CONTEXT GATHERING (Search & Ponder)
1. Open 'core\src\transport\swarm.rs' and read the implementation of 'transport_type_to_routing_transport'. Understand its parameters, return type, and exact purpose.
...
3. Identify EVERY valid location in the codebase where 'transport_type_to_routing_transport' MUST be invoked to work fully. 

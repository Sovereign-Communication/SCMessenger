...
The function 'force_ratchet' is defined in 'core\src\crypto\ratchet.rs' but is NEVER called anywhere in the repository. Your job is to fully integrate it into the overarching application loop. It may need to be wired into 1, 2, or 3+ different locations.

You MUST follow this strict analytical loop. Do not skip phases.

PHASE 1: CONTEXT GATHERING (Search & Ponder)
1. Open 'core\src\crypto\ratchet.rs' and read the implementation of 'force_ratchet'. Understand its parameters, return type, and exact purpose.
...
3. Identify EVERY valid location in the codebase where 'force_ratchet' MUST be invoked to work fully. 

...
TARGET: core/src/transport/wifi_aware.rs

Wiring task: 

The function 'add_discovered_peer' is defined in 'core/src/transport/wifi_aware.rs' but is NEVER called anywhere in the repository. Your job is to fully integrate it into the overarching application loop. It may need to be wired into 1, 2, or 3+ different locations.

PHASE 1: CONTEXT GATHERING
1. Open 'core/src/transport/wifi_aware.rs' and read the implementation of 'add_discovered_peer'. Understand its parameters, return type, and exact purpose.
2. Use search tools to hunt for related concepts, APIs, UI buttons, or parent managers where similar functions are called.
3. Identify EVERY valid location in the codebase where 'add_discovered_peer' MUST be invoked to work fully.

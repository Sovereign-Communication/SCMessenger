...
TARGET: core\src\transport\bootstrap.rs

SYSTEM DIRECTIVE: COMPREHENSIVE DEAD-END RESOLUTION
The function 'reset_circuit_breakers' is defined in 'core\src\transport\bootstrap.rs' but is NEVER called anywhere in the repository. Your job is to fully integrate it into the overarching application loop. It may need to be wired into 1, 2, or 3+ different locations.

You MUST follow this strict analytical loop. Do not skip phases.

PHASE 1: CONTEXT GATHERING (Search & Ponder)
1. Open 'core\src\transport\bootstrap.rs' and read the implementation of 'reset_circuit_breakers'. Understand its parameters, return type, and exact purpose.
...
3. Identify EVERY valid location in the codebase where 'reset_circuit_breakers' MUST be invoked to work fully. 

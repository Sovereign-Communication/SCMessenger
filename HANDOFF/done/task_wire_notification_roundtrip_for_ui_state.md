...
TARGET: wasm\src\daemon_bridge.rs

SYSTEM DIRECTIVE: COMPREHENSIVE DEAD-END RESOLUTION
The function 'notification_roundtrip_for_ui_state' is defined in 'wasm\src\daemon_bridge.rs' but is NEVER called anywhere in the repository. Your job is to fully integrate it into the overarching application loop. It may need to be wired into 1, 2, or 3+ different locations.

You MUST follow this strict analytical loop. Do not skip phases.

PHASE 1: CONTEXT GATHERING (Search & Ponder)
1. Open 'wasm\src\daemon_bridge.rs' and read the implementation of 'notification_roundtrip_for_ui_state'. Understand its parameters, return type, and exact purpose.
...
3. Identify EVERY valid location in the codebase where 'notification_roundtrip_for_ui_state' MUST be invoked to work fully. 

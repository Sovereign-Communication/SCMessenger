...
TARGET: core\src\routing\adaptive_ttl.rs

SYSTEM DIRECTIVE: COMPREHENSIVE DEAD-END RESOLUTION
The function 'calculate_dynamic_ttl' is defined in 'core\src\routing\adaptive_ttl.rs' but is NEVER called anywhere in the repository. Your job is to fully integrate it into the overarching application loop. It may need to be wired into 1, 2, or 3+ different locations.

You MUST follow this strict analytical loop. Do not skip phases.

PHASE 1: CONTEXT GATHERING (Search & Ponder)
1. Open 'core\src\routing\adaptive_ttl.rs' and read the implementation of 'calculate_dynamic_ttl'. Understand its parameters, return type, and exact purpose.
...
3. Identify EVERY valid location in the codebase where 'calculate_dynamic_ttl' MUST be invoked to work fully. 

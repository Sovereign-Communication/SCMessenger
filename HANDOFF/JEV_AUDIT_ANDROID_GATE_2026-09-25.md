...
- **Symptom.** Pull request #361 (head `63f4a7d73ef05ace437b54d11c14a256c71eb923`) has one red check: `Android JVM Unit Tests`, `BUILD FAILED in 20m 12s`. Its other 32 checks pass, including `Android Debug APK` and all four ABI builds.
- **The red check does not block the merge, and that is the material part.** The required status checks on `main` are exactly `Repository Hygiene Checks`, `Lint`, `Rust Linting`, `Test (ubuntu-latest)` and `Handoff ownership scope`. **`Android JVM Unit Tests` is not among them**, and there are no repository rulesets. On #361 four of the five required checks pass and the fifth has never reported, so on the checks this repository actually enforces the pull request is green. A red Android gate is a visible signal, not a safety net: nothing stops #361 from being merged onto `main` with these two tests in place. An earlier revision of this handoff described the failing gate as an obstacle to merging; that overstated the protection and understated the risk.
...
- **That fix is now verified green, not merely present.** `Android JVM Unit Tests`, job `108481836751`, run `36269937310`, head `2cb046cf`: conclusion `success`, 22m 55s, `BUILD SUCCESSFUL in 22m 33s`, 356 tests started, 353 passed, **0 failed**, 3 skipped, with both formerly-failing tests explicitly `PASSED`. So the acceptance signal requested below has already been observed once, on a branch that carries this correction.
- **Honest limitation of the proposed fix.** `any<Intent>()` in MockK matches a null argument, so the repaired test proves which context entry point was invoked and that the other was not, but no longer proves the action constant. That is a deliberate and correct trade for a JVM unit test in this module: the action value is unobservable here because `returnDefaultValues = true` makes android.jar's `Intent.action` a stub returning `null`. A regression that called the right context entry point with the wrong action string would now pass. Restoring action-level coverage needs an instrumented test or Robolectric reinstated for this class, which is separate work.

- **Scope correction, and what would actually close it.** This finding names pull request #361. A later sweep enumerated all 49 open pull requests and found the same two assertions in a byte-identical test file (`sha256:0c4aff33…`) on **#359** and **#364** as well; all three are based on `main`, and #359 has never had the Android job run at all. Fixing #361 alone is not sufficient. The durable close is to add `Android JVM Unit Tests` to `required_status_checks.contexts` on `main`: the job already exists in `.github/workflows/mobile.yml`, already runs on every pull request touching the relevant paths, and already completed in 22m 55s on run `36269937310`, so requiring it costs no build time the project does not already spend. `docs/rules/ANDROID.md:59-64` already lists a JVM unit test in this repository's own Pre-Merge Checklist, and the comment at `.github/workflows/mobile.yml:60-68` says the job was built to serve as that pre-merge gate. Only the enforcement is missing. One prerequisite: the job is behind `paths:` filters, and GitHub treats a required check that never reports as pending, so it must also be made to always report or every non-Android pull request would be wedged. Full detail, evidence and per-carrier analysis are in `HANDOFF/JEV_AUDIT_AMENDMENT_1_2026-09-26.md`.

**Requested owner action for Finding 1**

1. Treat the Android JVM failure as a test-observability defect owned by the pull request carrying the test, not a production regression. No product code change is warranted. Apply it to all three carriers, not #361 alone.
2. Land the test-file hunk of `e7466639` (or an equivalent test-only correction) so #361's Android gate can go green, and re-run `Android JVM Unit Tests` as the acceptance signal. That signal has been observed passing once already, on run `36269937310` at head `2cb046cf`.
...
1. Apply Finding 1 first, and treat it as a correctness item rather than a queue-clearance item. The check is **not** required on `main`, so the failing Android gate blocks nothing; the defect is one merge away from `main`, and from there it reaches every branch cut afterwards. The fix already exists in the repository and has been observed green.

...
## Worker and memory policy (dynamic admission update 2026-08-14)

Delegate 100% of project work to at most three concurrent direct workers; worker memory is task-sized and must be admitted dynamically through `scripts/resource_admission.py`. Before launch, estimate the full worker-plus-descendant peak, add the default 10% safety margin, read fresh host telemetry, and reserve against `tmp/lakes/active_workers.json`, which is shared across Prime sessions and terminal lanes. Bind and sample the full process tree, then release only after completion and descendant cleanup. An explicit authenticated human or terminal-operator directive may approve an exception worker for any stated purpose; that approval removes the former per-worker 384 MiB ceiling but never bypasses current-RAM admission, the 2 GiB headroom floor, the global reservation budget, process monitoring, one-build-at-a-time serialization, security gates, provenance separation, or the PR139 HARD NO-GO. Every dispatch states exactly one hypothesis, exact scope, canonical path or API request-shape anchor, acceptance test, stop condition, maximum tool calls, task kind, estimated peak, safety margin, requested reservation, registry ID, and any operator approval. Unknown telemetry or unavailable capacity is BLOCKED, not guessed. Keep source evidence separate from runtime evidence.
...
3. Re-measure HEAD/status, runtime `/version`, and process RSS read-only. Run `python3 scripts/resource_admission.py snapshot` and reconcile `tmp/lakes/active_workers.json` before any launch. Preserve the immutable source/runtime boundary.
...
- Stop condition: stop on any live endpoint requirement, ambiguity, mutation outside scope, unavailable resource reservation, unknown telemetry, or an exceeded task-specific allocation; also stop after 24 tool calls. Report PASS or BLOCKED without deployment.
...
- Stop condition: stop on ambiguity, any mutation or content-output requirement, unavailable resource reservation, unknown telemetry, or an exceeded task-specific allocation; also stop after 16 tool calls.
...
- Stop condition: stop on any need for a live Prime/SCM call, an unresolved double-send/crash window, unavailable resource reservation, unknown telemetry, or an exceeded task-specific allocation; also stop after 18 tool calls. Report PASS or BLOCKED.

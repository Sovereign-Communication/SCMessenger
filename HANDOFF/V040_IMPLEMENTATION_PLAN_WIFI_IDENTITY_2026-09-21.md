# V0.4.0 comprehensive implementation plan — identity / transport / WiFi delivery

Status: Active — **authoritative for implementing models** (no guesswork)
Date: 2026-09-21
...
4. Source tickets in `HANDOFF/todo/` (evidence history — **not** status authority)
5. Code on `origin/main` (verified 2026-09-21 commands below)

Audit inputs (product only — no other-lane work):
...
## 0. Canonical model (non-negotiable — implement to this)

| Concept | ONE definition |
|---|---|
| Contact / addressing identity | **Ed25519 public-key hex** (64 hex chars), store + UI + ledger write format |
| Network peer id | Derived via documented extract/parse helpers — **never** a second contact-address flavor |
...
| Inbound identity | Delegate gets **authenticated** `sender_public_key_hex` + canonical derived id — never raw payload `sender_id` |
| Outbox keys | Canonical hex at enqueue; flush/drain must resolve both spellings (`resolve_queue_key` / dual-drain) |

---

## 1. Verification matrix — ticket vs `origin/main` (2026-09-21)
...
|---|---|---|---|
| **CRYPTO-01** spoofable `sender_id` to delegate | OPEN (P1 2026-09-16) | **MOSTLY CLOSED** — `iron_core` builds `sender_public_key_hex` from `sender_pubkey`, sets `sender_id: canonical_peer_id` | **Verify + tests only** — do not re-implement; write/extend unit test that spoofed payload sender ≠ delegate identity; close ticket with cite |
| **TRN-03 / WASM own-topic** | OPEN same ticket | **CLOSED on main** — `own_peer_key_hex` + `is_ghost_peer_topic` in `swarm.rs` (both loops) | **Verify WASM test / residual ghost-guard**; no new subscribe code unless test proves gap |
| **CLI send hex vs peer id** | "Fixed (pending CI)" | **CLOSED on main** — `peer_id_from_contact_identifier` accepts hex + base58; comments match the filed defect | **Regression test** CLI path (unit/integration); live WP5 proof |
| **Contact recovery peer_id as pubkey** | Active | **PARTIAL** — `placeholder_or_derived_contact` derives key via `public_key_hex_from_libp2p_peer_id`; placeholder when missing + notes | **CLOSE residual**: never write PeerId into `public_key`; placeholder must fail closed for **send encrypt** (no fake encrypt to empty key); tests |
| **Swarm routing feed** | OPEN routing ticket | **CLOSED for swarm** — production `core_arc.routing_peer_seen` in `swarm.rs` ConnectionEstablished | Field proof in WP5; do not re-wire |
| **Non-swarm routing feed** (BLE/WiFiAware/WiFiDirect) | OPEN P2 | **OPEN** — `mobile_bridge.rs` has **no** `routing_peer_seen` | **WP2 implement** — exact feed + fail-closed block gate |
...
| **CLI-03 / CORE-02 outbox** | P1 2026-09-16 | **CLOSED** (#297 + #322 lineage) | None — residual was CO-B-001 |
| **Receipts do not converge live** | OPEN | **OPEN** | **WP4** |
| **Crypto fail vs claimed delivered** | OPEN | **OPEN** | **WP4** — honest status: transport ACK ≠ app delivery |
...
| **conn-limit multi-port** | Wave-1 ticket | **OPEN** (live WARNs) | Freebuff `V040_T_CONN_LIMITS_MULTIPORT.md` (Rule-8) — **feeds WP5** |

**Audit checks (operator brief):**

| # | Check | Result |
|---|---|---|
| 1 | Handoff + P0 = SCMessenger product only | **PASS** — no cross-lane items in those two files |
| 2 | P0 points at existing OPEN todos; no duplicate plan | **PASS** after this doc — implement from **this** matrix + P0 WP order; do not open another root-cause plan |
| 3 | WiFi not claimed fixed without WP5 3-node logs | **PASS** — WP5 mandatory before any "WiFi fixed" / 0.4.0 working-bar claim |
| 4 | Canonical model = ONE identity + ONE routing feed entry | **PASS** — §0 table |

---

## 2. Work packages — exact remaining work (no guesswork)

Order: **WP1 → WP2 → WP3 → WP4 → WP5**. One PR per WP (or smaller). Isolated worktrees.

### WP1 — Identity unification (close the remaining write-side holes)

**Already on main:** hex as contact key; CLI send resolver; CO-B-001 flush dual-drain; CRYPTO-01 delegate binding (verify only).

**Must implement:**

| ID | Task | Files (anchors — re-grep before edit) | Acceptance |
|---|---|---|---|
| WP1.1 | Contact recovery: derive real pubkey or **refuse encrypt path** | `core/src/contacts_bridge.rs` `placeholder_or_derived_contact` / `emergency_recover` | Test: recovery with unknown peer → contact either has derived hex **or** cannot be used as encrypt recipient (no PeerId-as-pubkey) |
...
| WP1.3 | CLI/UI/core/Android/WASM send all resolve hex→PeerId via **one** helper | `cli/src/main.rs` `peer_id_from_contact_identifier`, `core` export if needed | Unit tests: hex, base58, name→hex→send |
| WP1.4 | CRYPTO-01 / WASM **regression tests** | `core/src/iron_core.rs` / tests, wasm tests | Spoofed payload id ≠ delegate identity; wasm own-topic subscribe asserted |
...
| WP2.2 | Fail-closed | same as swarm block-check | Unknown/blocked peer → **no** feed; test |
...
| WP3.3 | Authenticated sender to delegate — **all** receive paths | `iron_core.rs` `receive_message` | No path passes payload-only `sender_id` as `sender_public_key_hex` |

**Gate:** Rule-8 for transport/ghost-guard if still in gated dirs; JEV canonical before DONE.

### WP4 — Delivery truth / receipts / wedge
...
### WP5 — 3-node log + live WiFi proof (exit gate for "WiFi fixed")

**Mandatory evidence attached to P0 umbrella ticket** — not a PR body claim.

| Step | Command / action |
|---|---|
| 1 | Same-SHA fleet (or documented delta) |
| 2 | Capture **failed** send window logs all 3 nodes (commands in P0 WP5) |
| 3 | After WP1–4: capture **success** window A↔B↔C WiFi (no cellular) |
| 4 | Trace: enqueue → identity resolve → transport → route → send → ack → UI |
| 5 | Attach paths under P0 ticket; mark UNVERIFIED legs explicitly |

**Do not claim WiFi fixed without step 3–4 evidence.**
...
git worktree add C:\Users\SCM\Documents\GitHub\Harness-jev-use origin/main

# Key (if live JEV): ~/.config/harness/jev.env  (JEV_API_KEY / jev_api_key)
# Without a key: local structural fallback only — record is_fallback=true
```

JEV API (harness `origin/main` `harness/jev.py`):
- `JevEvaluator(state, questions).evaluate(...)` → `JevEvaluationResult`
- `result.is_passing(min_confidence=0.70)` — **canonical completion** requires `True` **and** command evidence
- Cost: `jev_cost(input_tokens)` = input_tokens * 42 / 1e6 (do not invent usage.cost)
- Default questions: `diff_question_pack()` — for WP completion use **custom pack** in §3.3
- Code-owned facts (paths, tests, greps) are **not** re-judged by JEV — JEV answers only semantic canonical compliance

### 3.2 Required mechanical gates (always — code-owned)
...
| Grep canon | WP-specific greps in §2 tables |
...
acceptance list, command outputs, canon §0 rows this WP must uphold**.

```python
# Pseudocode — implement via harness JevEvaluator from origin/main
questions = {
  "canon_identity": {
    "type": "noul",
...
claim canonical complete until a keyed pass is recorded — mechanical gates
still required.
...
3. If still unclear, write `HANDOFF/freebuff/inbox/` PREMISE-WRONG / QUESTION — do not invent a new root cause.
...
or a `Harness-jev-use` worktree of origin/main) and evaluates the §3.3 pack.
...
`UNVERIFIED-JEV` — do not treat that as canonical DONE.
...
|---|---|---|
| 0 | Read this file + P0 umbrella + master plan | — |
| 1 | WP1 ticket (`V050_WP1_...`) | tests + JEV §3.3 |
...
| 7 | Orchestrator merge train + fleet redeploy + final 0.4.0 checklist | master plan §4 |
...
|---|---|
| Working mesh day-to-day | WP1–4 code + WP5 proof |
...
- Handoff + P0: **product-only** — PASS
- No duplicate root-cause plan — THIS doc is the implementation authority
- WiFi claim requires WP5 evidence — enforced
- Canonical model single identity + single routing feed — §0

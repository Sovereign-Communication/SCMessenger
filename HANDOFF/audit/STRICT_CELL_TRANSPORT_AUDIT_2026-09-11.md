...
| 442dcd7b | 18:00Z | never Win/AWS | â€” | **FAIL** |
| 87009dbb | 18:01Z | never Win/AWS | â€” | **FAIL** |
...
| C3 | `isPortLikelyBlocked(9001)` true on cellular â†’ AWS deprioritized | **FIXED** 9001 in `allowedStandardPorts` |
| C4 | `relayCircuitAddressesForPeer` `prioritizedNodes=emptyList()` â€” no Win-via-AWS circuit | **FIXED** seed from public relay multiaddrs on cellular |
...
| G4â€“G6 | contact supersede / history_sync / seed-dial | OPEN |
...
3. Stay on cellular â‰¥90s. Seat pulls all 3 logs.
**Pass bar:** phone `delivery_state=delivered` + Win/AWS `inbox_receive` timestamps **both inside** the cellular window (mesh.log `Network type changed: Wifi â†’ Cellular` â€¦ `Cellular â†’ Wifi`).

...
not on that path (and not in this file as a D1-D7 definition), it is not
being worked on for the tag.
...
| I-30 | **F7 fix is a wire-format change and must land before the tag.** Widening the routing hint `[u8;4]` -> `[u8;8]` alters `NeighborhoodSummary.reachable_hints`, which is gossiped via `NeighborhoodGossip` -- so old and new builds cannot exchange hints. Operator ruled Option B on 2026-08-31 | `core/src/routing/neighborhood.rs:75-76` | **RULED, sequencing constraint recorded.** Cheap now because there is no installed base (no APK ever published; newest public release v0.1.9, March). The moment v0.4.0 reaches a stranger this becomes a compatibility matrix rather than a coordinated 3-node restart -- same expiry logic as the release keystore |
| I-31 | **Branch protection `strict: true` serialises every merge and multiplies CI.** With "require branches up to date" on, each merge puts every other open PR `BEHIND`, forcing a rebase and a full re-run. Five merges on 2026-09-01 cost roughly four extra full matrices (~130 checks) | `gh api .../branches/main/protection --jq .required_status_checks.strict` -> `true` | **OPEN -- operator decision, no ticket yet.** Do NOT simply disable it: `strict` is what makes "green" mean "tested against the actual parent", which is the discipline this session enforced repeatedly. The real fix is a **GitHub merge queue** (batches PRs, tests them together, merges atomically) or fewer/larger PRs. Disabling `strict` trades CI cost for the possibility of a semantically-conflicting merge that git resolves cleanly and main then fails on |
| I-32 | **The node advertises an ephemeral NAT source port as its external address, so the mesh is told to reach it where nothing listens.** Windows `external_addrs` = `["147.81.41.188:7196", "192.168.0.121:9001"]` -- 7196 is an ephemeral source port; the real listen port is 9001 | `GET /api/diagnostics`; and the AWS node's first live `[SEED-DIAL]` sweep dialled `/ip4/147.81.41.188/tcp/7196` and got `Connection refused` | **TICKETED P0** -- `V040_T14_EPHEMERAL_PORT_ADVERTISED_AS_EXTERNAL.md`. Found the moment T1's seed dial went live: the ledger, seed dial and routing engine can all be correct and the mesh still fails, because the address being distributed is wrong at the source. Distinct from T13/F4, which governs what we accept from others; this is what we say about ourselves |
| I-33 | **F1 measured in production: the `locally_verified` primitive is 100% vacuous on a real node.** After #262's migration the Windows ledger holds exactly 1024 entries (the cap) and **all 1024 are `locally_verified: true`** -- because the legacy CLI flag meant "peer advertised it", not "we dialled it". Also 77 entries carry our OWN peer id inside the multiaddr and 238 are `p2p-circuit` forms, many malformed (`Missing destination peer id`), which the boot seed dial then burns its budget on | Ledger inspection on the live Windows node after redeploy at `67d19d3c` | **Confirms the Rule-8 review empirically.** The containment holds: `success_count` is 0 for all 1024, and both export paths require `> 0`, so **0 entries would be exported** -- nothing leaks today. But the disclosure rule is doing no work at all; it is held up entirely by a second predicate that was never designed as the guard. T13/F1's one-line fix (`locally_verified: e.is_bootstrap`) is what makes the primitive real |
